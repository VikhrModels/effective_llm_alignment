...
class GenerateExamplesCallback(TrainerCallback):
    def __init__(
        ...
        self.temp_dir_path = os.path.join(
            temp_dir_base, "callback_generate_examples_dir"
        )
    ...
    def on_evaluate(self, args, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        with PartialState().split_between_processes(self.samples) as samples:
            for example in tqdm(samples, desc="Generating examples"):
                input_ids = (
                    torch.tensor(example["input_ids"]).unsqueeze(0).to(model.device)
                )
                attention_mask = (
                ...
                prompt_text = self.tokenizer.decode(
                    input_ids.squeeze(), skip_special_tokens=False
                )
                completion_text = self.tokenizer.decode(
                    outputs.squeeze(), skip_special_tokens=False
                )[len(prompt_text) :]
        ...
        save_dataframe(
            pd.DataFrame(records), PartialState().process_index, self.temp_dir_path
        )
            ...
            if self.logger_backend == "clearml":
                ...
                if task:
                    ...
                    logger.report_table(
                        ...
                        table_plot=combined_df,
                    )
            elif self.logger_backend == "wandb":
                wandb.log(
                    {
                        f"eval/generated_text_{state.global_step}": wandb.Table(
